...
- Wren currently converts `Int64` to `Num`, so arbitrary 64-bit touch IDs can lose precision. General Wren enum constructors and lossless integers are tracked in git-bug `80d0ccf`; the existing Zyntax adapter gap is `c575125`.
